...
def savexyz(filename, p, w='w'):
    xyz = open(filename, w)
    xyz.write("%d\n" % len(p))
    xyz.write("kdb\n")
    for i in range(len(p)):
        xyz.write("%3s %10f %10f %10f\n" % (p.names[i], p.r[i][0], p.r[i][1], p.r[i][2]));
...
def loadxyz(filename):
    ...
    for line in lines[2:]:
        ...
        a.names[i] = split[0]
        a.r[i] = [float(split[1]), float(split[2]), float(split[3])]
        i += 1
    a.box = numpy.identity(3) * 1024 #XXX huge box - but is it huge enough?
    return a

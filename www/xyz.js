...
function xyz(canvasID) {
        ...
        for(var i = 0; i < self.queue.length; i++) {
            ...
            if(q[0] == "atom") {
                self.circle(Math.floor(q[1][0] * self.scale + self.canvas.width * 0.5), 
                            ...
                            Math.floor(q[2] * self.scale * 0.5), q[3]);
            }
        }
    }
    ...
    self.circle = function(x, y, radius, color) {
        self.ctx.lineWidth = 2.0 * (self.scale/64.0);
        self.ctx.beginPath();
        color[0] = (color[0]) * 0.9;
        color[1] = (color[1]) * 0.9;
        color[2] = (color[2]) * 0.9;
    ...
    self.canvas.onmousemove = self.event_mousemove;
    self.canvas.onmousedown = self.event_mousedown;
    self.canvas.onmouseup = self.event_mouseup;
